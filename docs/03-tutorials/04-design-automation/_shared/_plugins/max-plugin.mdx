...
> You may [download the Bundle ZIP](https://github.com/autodesk-platform-services/aps-design-automation-nodejs/tree/nodejs/bundles/UpdateMAXParam.zip) into the `bundles/` (Node.js) or `/designAutomationSample/wwwroot/bundles` (.NET 6) folder and [skip to **Upload Plugin Bundle** section](#upload-plugin-bundle).
