...
extracts its sub types to build the collection. Similarly, **Issue Root Cause** is a child of **Issue Root Cause Category**. 
...
No API yet to get [Issues Permission](https://help.autodesk.com/view/BUILD/ENU/?guid=Issues_Permissions) table like Forma for Construction UI does. Current API supports to fetch the logged user permission only.
