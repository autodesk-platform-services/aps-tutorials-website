...
> You may [download the Bundle ZIP](https://github.com/Autodesk-Forge/learn.forge.designautomation/raw/master/designAutomationSample/wwwroot/bundles/UpdateMAXParam.zip) into the `bundles/` (Node.js) or `/designAutomationSample/wwwroot/bundles` (.NET 6) folder and [skip to **Upload Plugin Bundle** section](#upload-plugin-bundle).
