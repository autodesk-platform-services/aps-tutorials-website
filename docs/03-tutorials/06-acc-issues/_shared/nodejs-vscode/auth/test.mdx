...
In this tutorial, **cookie-session** is used. It serializes the session data into 
a cookie which will include all the session data (like internal_token, refresh_token etc.). 
So, only **session** and *session.sig* are available on the cookie of client.
